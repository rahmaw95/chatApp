...
let _renderPodfileFromTemplateAsync = (() => {
  var _ref6 = _asyncToGenerator(function* (context, expoRootTemplateDirectory, sdkVersion, iosClientVersion) {
    const { iosProjectDirectory, projectName } = getPaths(context);
    const templatePodfilePath = _path.default.join(expoRootTemplateDirectory, 'template-files', 'ios', 'ExpoKit-Podfile');
    let reactNativeDependencyPath;
    if (context.type === 'user') {
      reactNativeDependencyPath = _path.default.relative(iosProjectDirectory, _path.default.join(context.data.projectPath, 'node_modules', 'react-native'));
    } else if (context.type === 'service') {
...
function getPaths(context) {
  ...
  if (context.type === 'user') {
    ...
    intermediatesDirectory = _path.default.join(iosProjectDirectory, '..', 'intermediates');
  } else {
