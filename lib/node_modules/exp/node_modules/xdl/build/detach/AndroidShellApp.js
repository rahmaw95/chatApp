...
let runShellAppModificationsAsync = exports.runShellAppModificationsAsync = (() => {
  var _ref7 = _asyncToGenerator(function* (context, isDetached = false) {
...
let downloadAssetsAsync = (() => {
  var _ref10 = _asyncToGenerator(function* (assets, dest) {
    yield fs.ensureDir(dest);
    const batches = _.chunk(assets, 5);
    for (const batch of batches) {
      yield Promise.all(batch.map((() => {
        var _ref11 = _asyncToGenerator(function* (asset) {
          if (!asset.fileHashes) {
            return;
          }
          const downloadTasks = asset.fileHashes.map((() => {
            var _ref12 = _asyncToGenerator(function* (hash, idx) {
              yield saveUrlToPathAsync('https://d1wp6m56sqw74a.cloudfront.net/~assets/' + hash, path.join(dest, 'asset_' + hash));
            });
  ...
  return function downloadAssetsAsync(_x15, _x16) {
