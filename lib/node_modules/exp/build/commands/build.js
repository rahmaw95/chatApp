...
exports.default = function (program) {
  program.command('build:ios [project-dir]').alias('bi').option('-c, --clear-credentials', 'Clear stored credentials.').option('-t --type <build>', 'Type of build: [archive|simulator].', /^(archive|simulator)$/i).option('--release-channel <channel-name>', 'Pull from specified release channel.', 'default').option('-f, --local-auth', 'Turn on local auth flow').option('--expert-auth', "Don't log in to Apple, provide all of the files needed to build.").description('Build a standalone IPA for your project, signed and ready for submission to the Apple App Store.').allowNonInteractive().asyncActionProjectDir(function (projectDir, options) {
    ...
    if (options.localAuth && options.expertAuth) {
      (_log || _load_log()).default.error('Only one authentication mode can be selected at once.');
      process.exit(1);
    }
